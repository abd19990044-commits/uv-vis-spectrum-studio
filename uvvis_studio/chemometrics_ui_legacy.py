...
def render_legacy_chemometrics(processed: list[dict]) -> None:
    ...
    if len(processed) < 2:
        ...
        return
    try: wl, Xraw, names = _align_spectra(processed)
    ...
    if Xraw.shape[0] < 10: st.warning("The current sample count is small for robust multivariate validation. Treat model metrics as exploratory and increase independent samples before reporting a final model.")
    with st.expander("Preprocessing pipeline", expanded=True):
        c1,c2,c3,c4=st.columns(4); use_snv=c1.toggle("SNV",False,key="chem_snv"); use_msc=c2.toggle("MSC",False,key="chem_msc"); detrend=c3.toggle("Linear detrend",False,key="chem_detrend"); center=c4.toggle("Mean center",False,key="chem_center")
        c5,c6,c7=st.columns(3); autoscale=c5.toggle("Autoscale variables",False,key="chem_autoscale"); deriv=c6.selectbox("Savitzky–Golay derivative",[0,1,2],index=0,key="chem_deriv"); sg_window=c7.number_input("SG window",3,101,11,2,key="chem_sg_window"); sg_poly=st.number_input("SG polynomial order",1,7,2,1,key="chem_sg_poly")
    try: X=preprocess_matrix(Xraw,use_snv=use_snv,use_msc=use_msc,detrend=detrend,mean_center=center,autoscale=autoscale,derivative_order=int(deriv),savgol_window=int(sg_window),savgol_polyorder=int(sg_poly))
    except Exception as exc: st.error(f"Preprocessing error: {exc}"); return
    ...
    with explore:
        method=st.selectbox("Decomposition",["PCA","ICA","NMF / MCR-like"],key="chem_decomp"); max_comp=max(1,min(X.shape[0],X.shape[1],20)); ncomp=st.slider("Components",1,max_comp,min(3,max_comp),key="chem_ncomp")
        try:
            dec=unsupervised_decomposition(X,method,ncomp); scores=np.asarray(dec["scores"])
            if method=="PCA":
                p=pca_analysis(X,ncomp); ev=100*np.asarray(p["explained_variance_ratio"]); st.dataframe(pd.DataFrame({"PC":np.arange(1,len(ev)+1),"Explained variance (%)":ev,"Cumulative (%)":100*np.asarray(p["cumulative_variance"])}),hide_index=True,use_container_width=True)
                if scores.shape[1]>=2:
                    f=go.Figure(go.Scatter(x=scores[:,0],y=scores[:,1],mode="markers+text",text=names,textposition="top center")); f.update_layout(template="plotly_white",title="PCA scores",xaxis_title=f"PC1 ({ev[0]:.2f}%)",yaxis_title=f"PC2 ({ev[1]:.2f}%)"); st.plotly_chart(f,use_container_width=True)
                    ld=np.asarray(p["loadings"]); lf=go.Figure(); lf.add_trace(go.Scatter(x=wl,y=ld[:,0],name="PC1 loading")); lf.add_trace(go.Scatter(x=wl,y=ld[:,1],name="PC2 loading")); lf.update_layout(template="plotly_white",xaxis_title="Wavelength (nm)",yaxis_title="Loading",title="PCA loadings"); st.plotly_chart(lf,use_container_width=True)
                st.dataframe(pd.DataFrame({"Sample":names,"Hotelling T²":p["hotelling_t2"],"Q residual":p["q_residual"]}),use_container_width=True,hide_index=True)
            else:
    ...
    target_table=pd.DataFrame({"Sample":names,"Y / concentration":[np.nan]*len(names),"Class":[""]*len(names)}); edited=st.data_editor(target_table,use_container_width=True,hide_index=True,key="chem_targets",disabled=["Sample"])
    with regtab:
        y=pd.to_numeric(edited["Y / concentration"],errors="coerce").to_numpy(float); good=np.isfinite(y); models=["PLS","PCR","Linear regression","Ridge","Lasso","Elastic Net","SVR (RBF)","KNN","Random Forest","Extra Trees","Gradient Boosting"]
        ...
        if np.count_nonzero(good)>=5:
            try:
                rr=regression_analysis(X[good],y[good],model_name,n_components=int(comp),cv_folds=int(cv)); a,b,c=st.columns(3); a.metric("CV R² / Q²",f"{rr.r2_cv:.5f}"); b.metric("RMSECV",f"{rr.rmse_cv:.6g}"); c.metric("MAECV",f"{rr.mae_cv:.6g}")
    ...
    with clstab:
        labels=edited["Class"].fillna("").astype(str).str.strip().to_numpy(); valid=labels!=""; models=["LDA","QDA","Logistic regression","SVM (RBF)","KNN","Gaussian Naive Bayes","Random Forest","Extra Trees","Gradient Boosting"]
        ...
        if np.count_nonzero(valid)>=6 and len(np.unique(labels[valid]))>=2:
            try: cr=classification_analysis(X[valid],labels[valid],model_name,cv_folds=int(cv)); a,b=st.columns(2); a.metric("CV accuracy",f"{cr.accuracy_cv:.4f}"); b.metric("Balanced accuracy",f"{cr.balanced_accuracy_cv:.4f}"); st.dataframe(pd.DataFrame(cr.confusion,index=cr.classes,columns=cr.classes),use_container_width=True)
            except Exception as exc: st.error(str(exc))
