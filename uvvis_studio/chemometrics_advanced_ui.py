...
def render_advanced_chemometrics(processed: list[dict]) -> None:
    ...
    st.caption(
        "Dataset splitting, component optimization, Y-randomization and wavelength/interval screening. "
        "For confirmatory performance claims, use an independent test set or nested validation."
    )
    if len(processed) < 5:
    ...
    spl, optt, randt, vart = st.tabs(
        ["Train/test splitting", "PLS optimization", "Y-randomization", "Variable selection"]
    )
    ...
    with optt:
        ...
        try:
            r = optimize_pls_components(Xg, yg, int(maxc), int(cv))
            st.metric("CV-selected components", r["best_components"])
            st.metric("Minimum screening RMSECV", f"{r['best_rmsecv']:.6g}")
            st.metric("Q² at selected component count", f"{r['best_r2_cv']:.5f}")
            st.warning(
                "The minimum RMSECV is used here to choose the number of latent variables. Do not report that same "
                "minimum as an unbiased external performance estimate; confirm on an independent set or nested CV."
            )
            d = pd.DataFrame(r["results"])
            f = go.Figure()
            f.add_trace(go.Scatter(x=d["components"], y=d["rmsecv"], mode="lines+markers", name="RMSECV"))
            f.update_layout(
    ...
    with randt:
        ...
        if st.button("Run Y-randomization", key="chem_rand_run"):
            try:
                r = y_randomization_test(Xg, yg, int(nc), int(perms), int(cv))
                a, b, c = st.columns(3)
                a.metric("Observed Q²", f"{r['observed_q2']:.5f}")
                b.metric("Permutation p", f"{r['p_value']:.5f}")
                c.metric("Null mean Q²", f"{r['null_mean']:.5f}")
                hist = go.Figure(go.Histogram(x=r["permuted_q2"], nbinsx=30))
                hist.add_vline(x=r["observed_q2"], line_dash="dash", annotation_text="Observed Q²")
                hist.update_layout(
                    template="plotly_white",
    ...
    with vart:
        ...
        with v1:
            ...
            thr = st.number_input("VIP threshold", 0.0, 10.0, 1.0, 0.05)
            try:
                vr = pls_with_vip(Xg, yg, int(nc))
                sel = vip_threshold_select(vr["vip"], wl, thr)
                st.write(f"Selected wavelengths: **{len(sel['indices'])} / {len(wl)}**")
                vf = go.Figure(go.Scatter(x=wl, y=vr["vip"], mode="lines"))
                vf.add_hline(y=thr, line_dash="dash")
                vf.update_layout(
                    template="plotly_white",
                ...
                st.dataframe(
                    pd.DataFrame({"Wavelength (nm)": sel["wavelengths"], "VIP": sel["vip"]})
        ...
        with v2:
            ...
            try:
                rr = interval_pls(Xg, yg, wl, int(intervals), int(nc), int(cv))
                table = pd.DataFrame(
                    [{k: v for k, v in row.items() if k != "indices"} for row in rr["results"]]
                ).sort_values("rmsecv")
