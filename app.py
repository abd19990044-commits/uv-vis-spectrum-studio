...
.stButton>button,.stDownloadButton>button{border-radius:10px;min-height:2.4rem}.note{padding:.65rem .8rem;border-left:4px solid #334155;background:#f8fafc;border-radius:8px;color:#334155}
</style>""",unsafe_allow_html=True)
st.markdown('<div class="hero"><h1>🔬 UV-Vis Spectrum Studio</h1><p>Integrated analytical spectroscopy, quantitation, transforms, stoichiometry and publication tools.</p></div>',unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if derivative_order>=3: st.warning("3D/4D strongly amplify noise. Smoothing remains OFF unless you explicitly enable it.")
...
tabs=st.tabs(["📈 Spectra","∂ Derivatives & AUC","〰 FFT / Wavelet","📏 Calibration","🧪 Stoichiometry","➕ Standard addition","📊 Data & metrics","💾 Publication export"])
with tabs[0]:
...
with tabs[4]:
    ...
    with mrtab:
        ...
        if np.count_nonzero(mm)>=6:
            try: mr=mole_ratio_method(mx[mm],my[mm]); st.metric("Segmented-regression breakpoint",f"{mr['breakpoint_ratio']:.6g}"); mf=go.Figure(go.Scatter(x=mr["x"],y=mr["y"],mode="markers",marker=dict(color="#111827",size=8),name="Data")); x1=np.linspace(np.min(mr["x"]),mr["breakpoint_ratio"],100); x2=np.linspace(mr["breakpoint_ratio"],np.max(mr["x"]),100); mf.add_trace(go.Scatter(x=x1,y=mr["intercept1"]+mr["slope1"]*x1,mode="lines",name="Segment 1",line=dict(color="#111827"))); mf.add_trace(go.Scatter(x=x2,y=mr["intercept2"]+mr["slope2"]*x2,mode="lines",name="Segment 2",line=dict(color="#111827",dash="dash"))); mf.update_layout(template="plotly_white",xaxis_title="Reagent/analyte molar ratio",yaxis_title="Response",title="Mole-ratio plot"); st.plotly_chart(mf,use_container_width=True,config={"displaylogo":False}); st.write(f"Segment R²: {mr['r2_1']:.6f} and {mr['r2_2']:.6f}")
            except Exception as exc: st.error(str(exc))
...
with tabs[5]:
    ...
    if np.count_nonzero(sm)>=3:
        try: sr=standard_addition(sx[sm],sy[sm],dilution_factor=dilution); s1,s2,s3,s4=st.columns(4); s1.metric("Slope",f"{sr['slope']:.8g}"); s2.metric("R²",f"{sr['r2']:.8f}"); s3.metric("x-intercept",f"{sr['x_intercept']:.8g}"); s4.metric("Original sample concentration",f"{sr['sample_concentration_original']:.8g}"); sf=go.Figure(); sf.add_trace(go.Scatter(x=sx[sm],y=sy[sm],mode="markers",name="Data",marker=dict(color="#111827",size=9))); xx=np.linspace(min(np.min(sx[sm]),sr['x_intercept']),np.max(sx[sm]),150); sf.add_trace(go.Scatter(x=xx,y=sr['intercept']+sr['slope']*xx,mode="lines",name="Fit",line=dict(color="#111827"))); sf.update_layout(template="plotly_white",xaxis_title="Added concentration",yaxis_title="Response",title="Standard-addition plot"); st.plotly_chart(sf,use_container_width=True,config={"displaylogo":False})
        except Exception as exc: st.error(str(exc))
    else: st.info("Enter at least three complete standard-addition points.")
with tabs[6]:
    if processed:
        md=pd.DataFrame(metrics).rename(columns={"lambda_max":"λmax (nm)","y_max":"Signal at λmax","lambda_min":"λmin (nm)","y_min":"Signal at λmin","area":"Signed AUC full","absolute_area":"Absolute AUC full","centroid":"Centroid (nm)","fwhm":"FWHM (nm)","peak_count":"Peaks"}); st.markdown("#### Spectral metrics"); st.dataframe(md,use_container_width=True,hide_index=True); st.markdown("#### Detected peaks"); st.dataframe(pd.DataFrame(peak_rows),use_container_width=True,hide_index=True) if peak_rows else st.info("No peaks at the current prominence threshold."); st.markdown("#### Signal at selected wavelengths"); wtxt=st.text_input("Wavelengths (comma separated)","270",key="extract_wavelengths"); req=[]
...
with tabs[7]:
    if fig is None: st.info("Load spectra to export a spectral figure. Calibration/Job/standard-addition plots remain available interactively in their tabs.")
    else:
        e1,e2,e3=st.columns(3); win=e1.number_input("Width (in)",2.0,20.0,7.0,.25); hin=e2.number_input("Height (in)",2.0,20.0,5.0,.25); dpi=e3.select_slider("PNG DPI",[300,600,720,900,1200],600); d1,d2,d3,d4=st.columns(4)
        try:d1.download_button(f"PNG · {dpi} DPI",figure_png_bytes(fig,win,hin,int(dpi)),"uvvis_spectrum.png","image/png",use_container_width=True)
        except Exception as exc:d1.warning(str(exc))
        try:d2.download_button("SVG",figure_vector_bytes(fig,"svg",win,hin),"uvvis_spectrum.svg","image/svg+xml",use_container_width=True)
        except Exception:d2.button("SVG unavailable",disabled=True,use_container_width=True)
        try:d3.download_button("PDF",figure_vector_bytes(fig,"pdf",win,hin),"uvvis_spectrum.pdf","application/pdf",use_container_width=True)
...
st.divider(); st.caption("UV-Vis Spectrum Studio · analytical spectroscopy and quantitative-analysis workspace")
